...
class Car
{
    constructor(index, name, category, price, date, img){
        this.index = index;
        this.name = name;
...
class ItemList{
    ...
    static table = CarDB.table;
    static arr = [];
    static categories = [];
    static count = 0;

    static createItem(record){
        return new Car(
            ItemList.count++,
            record.CarModel,
            record.Category,
    ...
    static register(recordObj, arr){
        return arr.push(recordObj);
    }

    static toArray(){
        ItemList.arr = [];
        CarDB.table.forEach(record => ItemList.register((ItemList.createItem(record)), ItemList.arr));
    ...
    static getAllCategories(){
        CarDB.table.forEach(record => {
            if(!ItemList.categories.includes(record.Category)) ItemList.categories.push(record.Category);
        });
        return ItemList.categories;
    }

    static classifiedItemLists(category){
        if(category == "All") return ItemList.toArray();

        let arr = [];
        CarDB.table.forEach(record => {
            if(record.Category == category) ItemList.register(ItemList.createItem(record), arr);
        });
        console.log(arr);
        return arr;
    }

    static sort(itemList, requestSort){
        let result = []; ['Price: Low to High', 'Price: High to Low', 'Newest Arrivals'];
        switch(requestSort){
            case "Price: Low to High" :
                result = itemList.sort(function(a, b){
...
var app = new Vue({
    ...
    data: {
        category: 'Category',
        sort: 'Sort by:',
        itemCards: ItemList.toArray(),
    },
    computed: {
        getCategories(){
            return ItemList.getAllCategories();
        },
        sortItems(){
            return ['Price: Low to High', 'Price: High to Low', 'Newest Arrivals'];
        },
    },
    methods: {
        changeCategory(){
            this.itemCards = ItemList.classifiedItemLists(this.category);
            this.sortEvent();
        },
        sortEvent(){
            this.itemCards = ItemList.sort(this.itemCards, this.sort);
